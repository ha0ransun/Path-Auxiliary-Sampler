...
from PAS.model import BernoulliRBM
from PAS.data_util.data_loader import load_mnist
...
def main(args):
    ...
    gt_samples = model.gibbs_sample(n_steps=args.gt_steps, n_samples=args.n_samples, plot=True)
    ...
    for temp in temps:
        ...
        ess[temp] = get_ess(chain, 1 - args.ess_ratio)
        chains[temp] = chain
        log = "temp {}, succ = {:.4f}, movements = {:.2f}, ess = {:.2f} +/- {:.2f}".format(temp, sampler.avg_accs,
                                                                                           sum(hops[temp]) / len(hops[temp]), ess[temp].mean(), ess[temp].std())
        print(log)
        if not os.path.isdir('results'):
            os.mkdir('results')
        with open(os.path.join(args.save_dir, f"results/result_{args.seed}.txt"), 'a') as handle:
            handle.write(log + '\n')
    ...
    with open("results/rbm_results.pkl", 'wb') as f:
        results = {
            'ess': ess,
            'hops': hops,
            'log_mmds': log_mmds,
